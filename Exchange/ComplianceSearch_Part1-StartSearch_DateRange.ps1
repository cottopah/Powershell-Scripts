...
$StartDate = Read-Host "Enter start date of search (use MM/DD/YYYY format)"
$EndDate = Read-Host "Enter end date of search (use MM/DD/YYYY format)"

###
# Create search
###

New-ComplianceSearch -Name "$Name" -ContentMatchQuery "(From:$Sender) AND (Received:$StartDate..$EndDate) AND (Subject:\$Subject)" -ExchangeLocation All -Confirm:$false -Force | Out-Null
...
Start-Sleep -s 30
Get-ComplianceSearch -Identity "$Name" | Select Name,Status | FL
Write-Output "Proceed to Part2 script if status is 'Completed'. If it is not, continue to use ""Get-ComplianceSearch -Identity $Name"" until you see it as 'Completed'. Then proceed with second script to perform delete action."