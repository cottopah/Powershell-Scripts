...
# Create Function to connect to Exchange on-premises
...
###
# Connect to Exchange on-premises and AD on-premises
###
...
###
$CompanyBrand = "IPC"
$CompanyBrandCountry = "UK"
$CompanyCountryCode = "GB"
$CompanyName = "Vaclensa"
$CompanyOffice = "Bolton"
$ADUserUPNSuffix = "vaclensa.com"
$ADUserOU = "OU=Vaclensa,OU=IPC,OU=Resource Mailboxes,OU=Mail Contacts and Resource Mailboxes,DC=tennant,DC=tco,DC=corp"
$SharedMBXEmailDomain = "vaclensa.com"
$ADGroupOU = "OU=Security,OU=Vaclensa,OU=IPC,OU=User Account Groups,OU=Account Groups,DC=tennant,DC=tco,DC=corp"
...
# Create mailbox
###
Enable-Mailbox -Identity $ADUserSamName -Shared | Out-Null
# Write output
Write-Output "`n"
...
###
New-ADGroup -DisplayName "$SecGroupDisplayName" -Name $SecGroupName -GroupCategory Security -GroupScope Universal -Path "$ADGroupOU" -Server $pdce -Description "Access to mailbox: $SharedMBXEmailAddress" | Out-Null
# Write output
Write-Output "`n"
